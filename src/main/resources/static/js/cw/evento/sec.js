			...
			var iFrameBody = '';
				...
				$(function() {
					...
					$("#btnNuevo").click(function () {
				    	window.location.href = _ctx+'gestion/evento/nuevo';
				    });
				...
				function listarRegistros() {
				    ...
				    if ($("#frm_busqueda").valid()) {
				        ...
				        $table.bootstrapTable({
				        	url: _ctx+lang+'gestion/evento/obtenerListado/'+comodin+'/'+estado,
				        	pagination: true,
				...
				function registro() {
				    if ($("#frm_registro").valid()) {
				        ...
				        $.ajax({
				            ...
				            url: _ctx+lang+'gestion/evento/agregar',
				            dataType: "json",
				            data: params,
				    		...
				    		$.smallBox({
								content : "<i> La <strong> descripcion del evento no puede actualizarse como vacío.</strong></i>",
								color: "#8a6d3b",
				...
				function editar(id) {
				    ...
				    $.ajax({
				        type: 'GET',
				        contentType : "application/x-www-form-urlencoded; charset=UTF-8",    
				        url: _ctx+lang+'gestion/evento/obtener',
				        dataType: "json",
				        data: param,
				        success: function (data, textStatus) {
				            if (textStatus == "success") {
				                $("#hIdEvento").val(data["id"]);
				                $("#TituloPrincipal").val(data["tituloPrincipal"]);
				                $("#TituloLargo").val(data["tituloLargo"]);
								iFrameBody.innerHTML = data["descripcion"];
								var fechaEvento = data["fechaEvento"].split("-");
								$("#Mes").val(parseInt(fechaEvento[1])-1);
								$("#Anio").val(fechaEvento[0]);
						...
						$.SmartMessageBox({
							...
							if (ButtonPressed === "Si") {
					            ...
					            $.ajax({
					                type: 'PUT',
					    	        contentType : "application/x-www-form-urlencoded; charset=UTF-8",    
					                url: _ctx+'gestion/evento/desactivar',
					                dataType: "json",
					                data: param,
					                success: function (data, textStatus) {
					                    if (textStatus == "success") {
				...
				function Opciones(value, row, index) {
					...
					opciones = `<a href='#' onclick='javascript:desactivar(${row.id}, ${index});' title='Actualizar status'><i class='glyphicon glyphicon-refresh'></i></a> &nbsp
								...
								<a href='#' onclick='javascript:eliminar(${row.id}, ${index});' title='Eliminar evento'><i class='glyphicon glyphicon-trash text-danger'></i></a> &nbsp`;
		            return  opciones;
		        }
				...
				function obtenerArchivos(id){
					...
					$tableFiles.bootstrapTable({
						url: _ctx+'gestion/evento/obtenerArchivos',
						pagination: false,
						sidePagination: 'client',
						pageSize: 10,
						queryParams: p => { return {eventoId: id};},
				...
				function obtenerImagenes(id){
					...
					$tableFiles.bootstrapTable('destroy');
					$tableFiles.bootstrapTable({
						url: _ctx+'gestion/evento/obtenerImagenes',
						pagination: false,
						sidePagination: 'client',
						pageSize: 10,
						queryParams: p => { return {eventoId: id};},
				...
				function visualizar(ruta){
					$('#Image').attr('src', _ctx+'media/image/evento'+ruta);
					$('#myModalImage').modal('show');
				}
				
				function descargar(ruta){
					window.location.href = _ctx+'media/file/evento/gt/0'+ruta;
					$.smallBox({
				...
				function addFiles() {
			        ...
			        var data = new FormData();
			        var id = $('#hIdEvento').val();
			        			        
			        data.append("EventoId", id);
			        ...
			        $.ajax({
			            type: 'POST',
			            url: _ctx+'gestion/evento/cargarArchivos/v2',
			            data : data,
			            contentType : false,
			            processData : false,
	                    xhr: function() {
				...
				function confirmarEliminacion(id, ix){
					...
					$.ajax({
			            type: 'DELETE',
			            contentType : "application/x-www-form-urlencoded; charset=UTF-8",    
			            url: _ctx+'gestion/evento/eliminar/'+id,
			            dataType: "json",
			            success: function (data, textStatus) {
			                if (textStatus == "success") {   
			                	if(data=="-9" || data=="-88"){
